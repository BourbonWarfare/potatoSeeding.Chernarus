author = "Lambda.Tiger";
description = "Chernarus Defense Forces (AK-107, late 1990s) AI";
#include "../undef.hpp"
#include "../undef_side_gear.hpp"
#define SIDE_BASE_GRENADES "gm_handgrenade_frag_rgd5"
#define SIDE_LEADER_GRENADES SIDE_BASE_GRENADES,"rhs_mag_rdg2_white","CUP_1RND_SMOKE_GP25_M"
#define SIDE_FAC_GRENADES "rhs_mag_rdg2_white:2","gm_handgrenade_frag_rgd5:2","1Rnd_SmokeBlue_Grenade_shell:4","1Rnd_SmokeYellow_Grenade_shell:3","1Rnd_SmokeOrange_Grenade_shell:3","1Rnd_SmokeGreen_Grenade_shell:5"
#define SIDE_MEDIC_GRENADES "rhs_mag_rdg2_white:2","SmokeShellPurple"
#define SIDE_CREW_GRENADES "rhs_mag_rdg2_white"
#define SIDE_BINOS "gm_df7x40_blk"
#define CAMO_UNIFORM "CUP_U_B_CDF_MNT_1"
#define CAMO_VEST "CUP_V_CDF_6B3_1_MNT","CUP_V_CDF_6B3_2_MNT"
#define CAMO_VEST_AR "CUP_V_CDF_6B3_3_MNT"
#define CAMO_VEST_FTL "CUP_V_CDF_6B3_4_MNT"
#define CAMO_VEST_SL "CUP_V_CDF_6B3_5_MNT"
#define CAMO_BACKPACK "B_Battle_Belt_XL_F"
#define CAMO_HEADGEAR "CUP_H_CDF_H_PASGT_MNT"
#define CAMO_HEADGEAR_SPECIAL "CUP_H_CDF_H_PASGT_MNT"
#define CARRYALL_MAT "CUP_B_RPGPack_Khaki"
#define CARRYALL_MMG "gm_ge_army_backpack_90_oli"
#define CARRYALL_HAT "gm_ge_army_backpack_90_oli"
#define CARRYALL_MEDIC "cwr3_o_backpack_rd54_medic_empty"
#define CAMO_UNIFORM_PILOT "CUP_U_B_CDF_MNT_1"
#define CAMO_VEST_PILOT "CUP_V_CDF_OfficerBelt"
#define CAMO_HEADGEAR_PILOT "CUP_H_RUS_ZSH_Shield_Up"
#define CAMO_BACKPACK_PILOT CAMO_BACKPACK
#define CAMO_UNIFORM_VICC "CUP_U_B_CDF_MNT_2"
#define CAMO_VEST_VICC "CUP_V_CDF_CrewBelt"
#define CAMO_HEADGEAR_VICC "CUP_H_RUS_TSH_4_Brown"
#define CAMO_BACKPACK_VICC CAMO_BACKPACK
#define RIFLE "CUP_arifle_AK107_railed"
#define RIFLE_MAG "CUP_30Rnd_545x39_AK74M_M:3","CUP_30Rnd_TE1_Green_Tracer_545x39_AK74M_M:3"
#define RIFLE_ATTACHMENTS
#define AAR_ATTACHMENTS RIFLE_ATTACHMENTS
#define ALT_OPTICS
#define GLRIFLE "CUP_arifle_AK107_GL_railed"
#define GLRIFLE_MAG RIFLE_MAG
#define GLRIFLE_MAG_SMOKE "CUP_1RND_SMOKE_GP25_M","CUP_1Rnd_SmokeRed_GP25_M"
#define GLRIFLE_MAG_HE "CUP_1Rnd_HE_GP25_M"
#define CARBINE "CUP_arifle_AK105_railed"
#define CARBINE_MAG RIFLE_MAG
#define AR "CUP_arifle_RPK74M_railed"
#define AR_MAG "CUP_45Rnd_TE4_LRT4_Green_tracer_545x39_RPK74M_M:4"
#define AT "rhs_weap_rpg26"
#define AT_MAG
#define MMG "CUP_lmg_PKM_top_rail_B50_vfg"
#define MMG_MAG "CUP_100Rnd_TE4_LRT4_762x54_PK_Tracer_Green_M:5"
#define MAT "gm_rpg7_wud"
#define MAT_MAG "gm_1Rnd_40mm_heat_pg7vl_rpg2","gm_1Rnd_40mm_heat_pg7v_rpg7"
#define MAT_MAG3 "gm_1Rnd_40mm_heat_pg7vl_rpg7","gm_1Rnd_40mm_heat_pg7v_rpg7"
#define MAT_OPTIC "gm_pgo7v_blk"
#define HMG "CUP_KORD_carry"
#define HMG_TRI_HI "ace_csw_kordCarryTripod"
#define HMG_TRI_LO "ace_csw_kordCarryTripod"
#define HMG_MAG "ace_csw_50Rnd_127x108_mag"
#define HAT "potato_cswCompatCUP_patchGM_gm_fagot_backpack"
#define HAT_TRI_HI
#define HAT_TRI_LO
#define HAT_MAG "gm_1Rnd_fagot_heat_9m111_csw"
#define SAM "gm_9k32m_oli"
#define SAM_MAG "gm_1Rnd_72mm_he_9m32m:2"
#define SNIPER "CUP_srifle_AWM_blk"
#define SNIPER_MAG "CUP_5Rnd_86x70_L115A1:20"
#define SNIPER_ATTACHMENTS "CUP_optic_LeupoldM3LR","CUP_bipod_Harris_1A2_L_BLK"
#define SPOTTER "CUP_arifle_HK417_20"
#define SPOTTER_MAG "CUP_20Rnd_762x51_HK417:15"
#define SPOTTER_ATTACHMENTS "CUP_optic_Elcan_SpecterDR_RMR_black","CUP_bipod_Harris_1A2_L_BLK"
#define SMG "CUP_arifle_AKS74U"
#define SMG_MAG "CUP_30Rnd_545x39_AK74_plum_M:5"
#define SMG_MAG_FULL SMG_MAG
#define TOOLS "rhs_mag_rdg2_white"
#define LEADER_TOOLS TOOLS
#define LINKED BASE_LINKED
#define LEADER_LINKED BASE_LINKED

// ------------- Vehicle Pool -------------
transportVehiclePool[] = {
  "CUP_B_Tigr_233011_CDF",
  "CUP_B_Kamaz_CDF",
  "CUP_B_UAZ_Unarmed_CDF",
  "CUP_B_Ural_CDF"
};
armedSoftVehiclePool[] = {
  "CUP_B_Tigr_233014_PK_CDF",
  "CUP_B_UAZ_MG_CDF",
  "CUP_B_UAZ_AGS30_CDF"
};
armoredVehiclePool[] = {
  "CUP_B_BTR80_CDF",
  "CUP_B_MTLB_pk_CDF",
  "CUP_B_BRDM2_CDF"
};
tankVehiclePool[] = {
  "CUP_B_BTR80A_CDF",
  "CUP_B_BMP2_CDF",
  "CUP_B_T72_CDF"
};
reinforcementTruck = "CUP_B_Ural_CDF";
reinforcementArmed = "CUP_B_BTR80_CDF";
// ------------- End Vehicle Pool -------------

class Car {};
class Tank {};
class Helicopter {};
class Plane {};
class Ship_F {};
class rifleman {
  uniform[] = {CAMO_UNIFORM};
  vest[] = {CAMO_VEST};
  headgear[] = {CAMO_HEADGEAR};
  backpack[] = {CAMO_BACKPACK};
  weapons[] = {RIFLE};
  magazines[] = {RIFLE_MAG,SIDE_BASE_GRENADES};
  items[] = {TOOLS,BASE_MEDICAL};
  linkedItems[] = {LINKED};
  attachments[] = {RIFLE_ATTACHMENTS};
  opticChoices[] = {ALT_OPTICS};
};
class rifleman_02: rifleman {};
class Fic_Soldier_Carbine: rifleman {
  weapons[] = {CARBINE};
  magazines[] = {CARBINE_MAG,SIDE_BASE_GRENADES};
};
class ftl: rifleman {
  weapons[] = {GLRIFLE};
  magazines[] = {GLRIFLE_MAG,GLRIFLE_MAG_HE,GLRIFLE_MAG_SMOKE,SIDE_LEADER_GRENADES};
  vest[] = {CAMO_VEST_FTL};
  items[] += {LEADER_TOOLS};
  linkedItems[] += {SIDE_BINOS};
};
class sl: ftl {
  vest[] = {CAMO_VEST_SL};
  linkedItems[] = {LINKED,SIDE_BINOS};
  items[] = {TOOLS,LEADER_TOOLS,BASE_MEDICAL};
};
class coy: sl {
  linkedItems[] += {SIDE_BINOS};
};
class ar: rifleman {
  weapons[] = {AR};
  vest[] = {CAMO_VEST_AR};
  magazines[] = {AR_MAG,SIDE_BASE_GRENADES};
};
class aar: rifleman {
  backpackItems[] += {AR_MAG};
  linkedItems[] += {SIDE_BINOS};
};
class lat: Fic_Soldier_Carbine {
  magazines[] += {AT_MAG};
  weapons[] = {RIFLE};
  launchers[] = {AT};
};
class sm: Fic_Soldier_Carbine {
  magazines[] = {CARBINE_MAG,SIDE_MEDIC_GRENADES};
  backpack[] = {CARRYALL_MEDIC};
  backpackItems[] = {MEDIC_MEDICAL_AI};
};
class Fic_Spotter: rifleman {
  linkedItems[] += {SIDE_BINOS};
};
class mmgg: ar {
  weapons[] = {MMG};
  backpack[] = {CARRYALL_MMG};
  magazines[] = {MMG_MAG,SIDE_BASE_GRENADES};
  attachments[] = {};
};
class mmgag: Fic_Spotter {
  backpackItems[] += {MMG_MAG};
  backpack[] = {CARRYALL_MMG};
};
class matg: Fic_Soldier_Carbine {
  backpackItems[] = {};
  magazines[] += {MAT_MAG};
  items[] += {BASE_MEDICAL};
  launchers[] = {MAT};
  secondaryAttachments[] = {MAT_OPTIC};
};
class matag: Fic_Spotter {
  backpackItems[] = {};
  magazines[] += {MAT_MAG};
  items[] += {BASE_MEDICAL};
};
class msamg: Fic_Soldier_Carbine {
  SAM_GEAR(CAMO_BACKPACK,SAM_MAG)
  launchers[] = {SAM};
};
class msamag: Fic_Spotter {
  SAM_GEAR(CAMO_BACKPACK,SAM_MAG)
};
class mtrg: Fic_Soldier_Carbine {
  MORTAR_GEAR("B_Mortar_01_weapon_F")
};
class mtrag: Fic_Spotter {
  MORTAR_GEAR("B_Mortar_01_support_F")
};
class spotter: Fic_Spotter {
  weapons[] = {SPOTTER};
  magazines[] = {SPOTTER_MAG,SIDE_BASE_GRENADES};
  items[] += {"ACE_ATragMX","ACE_Kestrel4500"};
  attachments[] = {SPOTTER_ATTACHMENTS};
};
class sniper: spotter {
  weapons[] = {SNIPER};
  magazines[] = {SNIPER_MAG,SIDE_BASE_GRENADES};
  items[] = {TOOLS,"ACE_RangeCard",BASE_MEDICAL};
  linkedItems[] = {LINKED};
  attachments[] = {SNIPER_ATTACHMENTS};
};
class pilot {
  uniform[] = {CAMO_UNIFORM_PILOT};
  backpack[] = {CAMO_BACKPACK_PILOT};
  vest[] = {CAMO_VEST_PILOT};
  headgear[] = {CAMO_HEADGEAR_PILOT};
  weapons[] = {SMG};
  magazines[] = {SMG_MAG,SIDE_CREW_GRENADES};
  attachments[] = {RIFLE_ATTACHMENTS};
  items[] = {BASE_MEDICAL,TOOLS,LEADER_TOOLS};
  linkedItems[] = {LINKED};
  opticChoices[] = {ALT_OPTICS};
};
class vicc: Fic_Soldier_Carbine {
  weapons[] = {SMG};
  magazines[] = {SMG_MAG,SIDE_CREW_GRENADES};
  uniform[] = {CAMO_UNIFORM_VICC};
  headgear[] = {CAMO_HEADGEAR_VICC};
  vest[] = {CAMO_VEST_VICC};
  linkedItems[] = {LINKED};
  items[] += {BASE_MEDICAL};
};
class vicd: vicc {
  backpackItems[] = {"Toolkit"};
  linkedItems[] = {LINKED};
};
class Fic_eng: vicd {
  uniform[] = {CAMO_UNIFORM};
  vest[] = {CAMO_VEST};
  headgear[] = {CAMO_HEADGEAR};
  backpack[] = {CAMO_BACKPACK};
  items[] += {BASE_ENG};
  backpackItems[] = {};
};
class demo: Fic_eng {
  magazines[] += {BASE_EXP};
};
class mine: Fic_eng {
  magazines[] += {BASE_MINE};
  handguns[] = {MINE_DETECTOR};
};
class demol: Fic_eng {
  magazines[] += {BASE_EXP};
  backpackItems[] = {"Toolkit"};
};
class eng: Fic_eng {
  backpackItems[] = {"Toolkit","ACE_EntrenchingTool","ACE_Fortify","ACE_wirecutter"};
};
class fac: coy {
  magazines[] = {GLRIFLE_MAG,SIDE_FAC_GRENADES,"Laserbatteries"};
  linkedItems[] = {LINKED,"CUP_LRTV"};
};
class artl: sl {
  backpack[] = {"B_Kitbag_mcamo"};
  backpackItems[] += {BASE_ARTILLERY};
};
class artg: rifleman {
  backpackItems[] += {BASE_ARTILLERY};
};
class plm: sm {
  backpackItems[] = {MEDIC_MEDICAL_AI};
};
class cm: plm {};
class xo: coy {};
class plt: coy {};
class sgt: plt {};
class vicl: vicc {
  linkedItems[] + = {SIDE_BINOS};
};
class mmgl: sl {
  backpack[] = {CARRYALL_MMG};
  magazines[] += {MMG_MAG};
};
class matl: sl {
  backpack[] = {CARRYALL_MAT};
  magazines[] += {MAT_MAG3};
};
class hmgl: sl {
  weapons[] = {RIFLE};
  magazines[] = {RIFLE_MAG,SIDE_LEADER_GRENADES,HMG_MAG};
  backpack[] = {CARRYALL_HAT};
  launchers[] = {HMG_TRI_LO};
  items[] += {BASE_BALLISTICS};
};
class hmgg: rifleman {
  backpack[] = {CARRYALL_HAT};
  magazines[] += {HMG_MAG};
  launchers[] = {HMG};
};
class hmgag: rifleman {
  backpack[] = {CARRYALL_HAT};
  magazines[] += {HMG_MAG};
  launchers[] = {HMG_TRI_HI};
  items[] += {BASE_BALLISTICS};
};
class hatl: sl {
  weapons[] = {RIFLE};
  backpackItems[] = {};
  magazines[] = {RIFLE_MAG,SIDE_LEADER_GRENADES,HAT_MAG};
  backpack[] = {CARRYALL_HAT};
  launchers[] = {HAT_TRI_LO};
  items[] += {BASE_MEDICAL};
};
class hatg: rifleman {
  backpack[] = {CARRYALL_HAT};
  magazines[] += {HAT_MAG};
  launchers[] = {HAT};
};
class hatag: rifleman {
  backpack[] = {CARRYALL_HAT};
  magazines[] += {HAT_MAG};
  launchers[] = {HAT_TRI_HI};
};
class msaml: sl {
  backpack[] = {CARRYALL_HAT};
  magazines[] += {SAM_MAG};
};
class mtrl: sl {
  items[] += {BASE_ARTILLERY};
};
class helicrew: pilot {
  backpackItems[] = {"Toolkit"};
};
class cc: helicrew {};
class engl: eng {
  weapons[] = {GLRIFLE};
  magazines[] = {GLRIFLE_MAG,GLRIFLE_MAG_HE,GLRIFLE_MAG_SMOKE,SIDE_LEADER_GRENADES};
  items[] += {LEADER_TOOLS};
  linkedItems[] += {SIDE_BINOS};
};
class uav: rifleman {};
class fallback: rifleman {};
#undef SIDE_BASE_GRENADES
#undef SIDE_LEADER_GRENADES
#undef SIDE_FAC_GRENADES
#undef SIDE_MEDIC_GRENADES
#undef SIDE_CREW_GRENADES
#undef SIDE_BINOS